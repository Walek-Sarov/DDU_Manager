...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВидимостьКомандВариантовОтчетов") И Параметры.ВидимостьКомандВариантовОтчетов = Ложь Тогда
		Элементы.КомандыВариантовОтчета.Видимость = Ложь;
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Иначе
			СуффиксФормыИсточникаИзменений = "";
		КонецЕсли;
	#Иначе
...
Процедура Подключаемый_СтандартныйПериод_НачалоПериода_ПриИзменении(Элемент)
	...
	ЭтаФорма[Элемент.Имя] = НачалоДня(ЭтаФорма[Элемент.Имя]);
	
	Настройки = СкомпоноватьНастройкиЭлемента(Элемент, Ложь, Ложь);
	Настройки.Пользовательская.Использование = Истина;
	Настройки.Пользовательская.Значение.ДатаНачала = ЭтаФорма[Элемент.Имя];
...
Процедура Подключаемый_СтандартныйПериод_КонецПериода_ПриИзменении(Элемент)
	...
	Настройки = СкомпоноватьНастройкиЭлемента(Элемент, Ложь, Ложь);
	Настройки.Пользовательская.Использование = Истина;
	Настройки.Пользовательская.Значение.ДатаОкончания = ЭтаФорма[Элемент.Имя];
...
Процедура НастроитьОтчет(Команда)
	...
	ПараметрыФормы.Вставить("ПредставлениеВарианта",                 ОтчетНаименованиеТекущегоВарианта);
...
Процедура ВыбратьПериодВОтдельнойФорме(ЭлементИлиКоманда)
	...
	Если Настройки.Пользовательская.Значение.ДатаНачала = РезультатВыбора.НачалоПериода
		И Настройки.Пользовательская.Значение.ДатаОкончания = РезультатВыбора.КонецПериода Тогда
		Возврат;
	КонецЕсли;
	
	Настройки.Пользовательская.Значение.ДатаНачала    = РезультатВыбора.НачалоПериода;
	Настройки.Пользовательская.Значение.ДатаОкончания = РезультатВыбора.КонецПериода;
	Настройки.Пользовательская.Использование = Истина;
	...
	ЭтаФорма["Период_Произвольный_Начало_"    + Настройки.ЭлементИдентификатор] = РезультатВыбора.НачалоПериода;
	ЭтаФорма["Период_Произвольный_Окончание_" + Настройки.ЭлементИдентификатор] = РезультатВыбора.КонецПериода;
...
Функция СкомпоноватьНастройкиЭлемента(ИмяИлиЭлементИлиКоманда, УстановитьИспользование, УстановитьЗначение)
	...
	Если УстановитьЗначение Тогда
		ТипОбщейНастройки = ТипЗнч(Настройки.Общая);
...
Процедура ВидимостьДоступностьКорректность(Изменения = "")
	...
	Если Изменения = "" ИЛИ Изменения = "СкрытьПоказатьБыстрыеНастройки" Тогда
		Элементы.КомандыВариантовПользовательскихНастроек.Видимость = РежимВариантаОтчета;
		Элементы.КомандыВариантовОтчета.Видимость = РежимВариантаОтчета;
		Элементы.БыстрыеНастройки.Видимость = РежимВариантаОтчета;
		...
		Если РежимВариантаОтчета Тогда
			...
			Иначе
				...
				Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("ВидимостьБыстрыхНастроек", ВидимостьБыстрыхНастроек);
				Если ВидимостьБыстрыхНастроек = Истина Тогда
					...
					Элементы.СкрытьПоказатьБыстрыеНастройки.Заголовок = НСтр("ru = 'Скрыть быстрые настройки'");
				Иначе
					Элементы.БыстрыеНастройки.Видимость = Ложь;
					Элементы.СкрытьПоказатьБыстрыеНастройки.Пометка = Ложь;
					Элементы.СкрытьПоказатьБыстрыеНастройки.Заголовок = НСтр("ru = 'Показать быстрые настройки'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура БыстрыеНастройкиЗагрузитьСхемуИОчиститьФорму(НовыеНастройки, СуффиксФормыИсточникаИзменений = "")
	...
	Если ТипЗнч(НовыеНастройки) = Тип("КомпоновщикНастроекКомпоновкиДанных") Тогда
		...
		//Отчет.КомпоновщикНастроек = НовыеНастройки;
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправления()
	...
	Для Каждого ПользовательскаяНастройка Из ПользовательскиеНастройкиКД.Элементы Цикл
		...
		Если ТипОбщейНастройки = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			...
			ДоступнаяНастройка  = ФункцииОтчетовКлиентСервер.ПолучитьДоступноеПоле(НастройкиКД.ДоступныеПоляОтбора, ОбщаяНастройка.ЛевоеЗначение);
		ИначеЕсли ТипОбщейНастройки = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
			СтрокаКэша.ЭтоПараметр = Истина;
			СтрокаКэша.ПолеКД      = Новый ПолеКомпоновкиДанных("ПараметрыДанных." + Строка(ОбщаяНастройка.Параметр));
			СтрокаКэша.Общая       = ОбщаяНастройка;
			ДоступнаяНастройка     = ФункцииОтчетовКлиентСервер.ПолучитьДоступныйПараметр(НастройкиКД.ПараметрыДанных.ДоступныеПараметры, ОбщаяНастройка.Параметр); 
		Иначе
		...
		СменитьТипНаГруппуЭлементовОтбора = Ложь;
		...
		ИначеЕсли ЗначениеЗаполнено(СтруктураПредставлений.Представление) Тогда
			...
			СменитьТипНаГруппуЭлементовОтбора = Истина;
		...
		Если ТипОбщейНастройки = Тип("ГруппаЭлементовОтбораКомпоновкиДанных")
			...
			// Флажок
			ИспользованиеОтбора = Элементы.Добавить(ИспользованиеИмя, Тип("ПолеФормы"), Элементы.ГруппыОтборов);
		...
		ИначеЕсли ТипОбщейНастройки = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			...
			Для Каждого Тип Из ДоступнаяНастройкаТипы Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ОтборОписаниеТипов = Новый ОписаниеТипов(ДоступнаяНастройка.ТипЗначения, , ВычитаемыеТипы);
			...
			Если ЭтоГрупповойВидСравнения Тогда
				ТипыСписковЗначений.Вставить("Отбор_" + ЭлементИдентификатор, ОтборОписаниеТипов);
			...
			Если ЭтоГрупповойВидСравнения Тогда
				...
				ГруппаОтбора = Элементы.Добавить("ГруппаОтбора_" + ЭлементИдентификатор, Тип("ГруппаФормы"), Элементы.СпискиОтборов);
			...
			Иначе
				...
				// Элементы
				ФлажокИспользование = Элементы.Добавить(ИспользованиеОтбораИмя, Тип("ПолеФормы"), Элементы.ЛевоеЗначение);
				ФлажокИспользование.Вид                = ВидПоляФормы.ПолеФлажка;
				ФлажокИспользование.Заголовок          = ЭлементЗаголовок + ":";
				ФлажокИспользование.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
				ФлажокИспользование.УстановитьДействие("ПриИзменении", "Подключаемый_ЭлементОтбораКомпоновкиДанных_Использование_ПриИзменении");
				
				//Элементы.ЛевоеЗначениеОтбора.Ширина = Макс(СтрДлина(ЭлементЗаголовок), Элементы.ЛевоеЗначениеОтбора.Ширина);//Мин(, 20);
				
				ЗначениеОтбора = Элементы.Добавить(ОтборИмя, Тип("ПолеФормы"), Элементы.ПравоеЗначение);
				...
				//ЗначениеОтбора.РастягиватьПоВертикали = Ложь;
				////ЗначениеОтбора.РастягиватьПоВертикали = Ложь;
				ЗначениеОтбора.КнопкаВыбора        = Истина;
				ЗначениеОтбора.КнопкаОткрытия      = Ложь;
				ЗначениеОтбора.КнопкаОчистки       = Истина;
				ЗначениеОтбора.КнопкаРегулирования = Ложь;
				ЗначениеОтбора.КнопкаСпискаВыбора  = Ложь;
				//ЗначениеОтбора.Ширина              = ?(ДоступнаяНастройка.ТипЗначения.СодержитТип(Тип("Дата")), 24, 20);
		...
		ИначеЕсли ТипОбщейНастройки = Тип("ЭлементУсловногоОформленияКомпоновкиДанных") Тогда
			...
			Если НЕ ЗначениеЗаполнено(ЭлементЗаголовок) Тогда
				ЭлементЗаголовок = Строка(ОбщаяНастройка.Оформление);
			КонецЕсли;
		...
		ИначеЕсли ТипОбщейНастройки = Тип("ВыбранныеПоляКомпоновкиДанных") Тогда
			...
			// Группы и элементы
			Группа = Элементы.Добавить("ГруппаПолейВыбора_" + ЭлементИдентификатор, Тип("ГруппаФормы"), Элементы.ВыбранныеПоля);
		...
		ИначеЕсли ТипОбщейНастройки = Тип("ПорядокКомпоновкиДанных") Тогда
			...
			// Группы и элементы
			Группа = Элементы.Добавить("ГруппаПорядка_" + ЭлементИдентификатор, Тип("ГруппаФормы"), Элементы.Порядки);
			ЗаполнитьЗначенияСвойств(Группа, ШаблонГруппыСФлажками);
			Группа.Заголовок = ЭлементЗаголовок + ":";
		...
		ИначеЕсли ТипОбщейНастройки = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
			...
			Иначе
				...
				ЗначениеПараметраБулево = (ТипЗнч(ОбщаяНастройка.Значение) = Тип("Булево"));
				...
				Иначе
					...
					ФлажокИспользование = Элементы.Добавить(ИспользованиеИмя, Тип("ПолеФормы"), Элементы.ЛевоеЗначение);
					ФлажокИспользование.Вид                = ВидПоляФормы.ПолеФлажка;
					ФлажокИспользование.Заголовок          = ЭлементЗаголовок + ":";
					ФлажокИспользование.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
				...
				Параметр = Элементы.Добавить(ЗначениеИмя, Тип("ПолеФормы"), ?(ИспользуетсяВсегда, Элементы.ПараметрыОтчета, Элементы.ПравоеЗначение));
				...
				Параметр.КнопкаОчистки       = НЕ ДоступнаяНастройка.ЗапрещатьНезаполненныеЗначения;
				...
				Если ИспользуетсяВсегда И ДоступнаяНастройка.ТипЗначения = Новый ОписаниеТипов("Булево") Тогда
					Параметр.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
				ИначеЕсли ИспользуетсяВсегда Тогда
		...
		ИначеЕсли ТипОбщейНастройки = Тип("СтруктураНастроекКомпоновкиДанных") Тогда
			...
			// Группы и элементы
			ГруппаГруппировки = Элементы.Добавить("ГруппаГруппировки_" + ЭлементИдентификатор, Тип("ГруппаФормы"), Элементы.СтруктурыНастроек);
...
Процедура БыстрыеНастройкиЗагрузитьЗначения()
	...
	Если НЕ ПользовательскиеНастройкиКД.ДополнительныеСвойства.Свойство("ВидимостьБыстрыхНастроек") Тогда
