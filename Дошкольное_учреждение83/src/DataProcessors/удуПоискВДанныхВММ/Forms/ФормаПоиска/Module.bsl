...
Функция ИскатьВыполнитьСервер(Направление, ТекущаяПозиция, СтрокаПоиска,МассивОтбор) Экспорт
	
	РазмерПорции = 20;
	
	СписокПоиска = ПолнотекстовыйПоиск.СоздатьСписок(СтрокаПоиска, РазмерПорции);
	// Дошкольное учреждение
	МассивОтборМетаданные = Новый Массив;
	Для Инкр = 0 По МассивОтбор.Количество() - 1 Цикл
		Если МассивОтбор[Инкр] = "Авторы" Тогда 
			МассивОтборМетаданные.Добавить(Метаданные.Справочники.удуАвторы);
		ИначеЕсли МассивОтбор[Инкр] = "Серии" Тогда 
			МассивОтборМетаданные.Добавить(Метаданные.Справочники.удуСерии);
		ИначеЕсли МассивОтбор[Инкр] = "Статьи" Тогда 
			МассивОтборМетаданные.Добавить(Метаданные.Справочники.удуСтатьи);
		ИначеЕсли МассивОтбор[Инкр] = "УчетныеЕдиницы" Тогда 
			МассивОтборМетаданные.Добавить(Метаданные.Справочники.удуУчетныеЕдиницы);
		ИначеЕсли МассивОтбор[Инкр] = "Многотомники" Тогда 
			МассивОтборМетаданные.Добавить(Метаданные.Справочники.удуМноготомники);
		КонецЕсли;
	КонецЦикла;
	
	СписокПоиска.ОбластьПоиска = МассивОтборМетаданные;
	//
...
// Сформировать массив объектов конфигурации, по которым будет осуществляться поиск.
&НаСервере
Функция ПолучитьМассивОтбор()
	// Дошкольное учреждение
	МассивОтбор = Новый Массив;
	Если Объект.ИскатьВАвторы Тогда
		МассивОтбор.Добавить("Авторы");
	КонецЕсли;
	Если Объект.ИскатьВУчетныеЕдиницы Тогда
		МассивОтбор.Добавить("УчетныеЕдиницы");
	КонецЕсли;
	Если Объект.ИскатьВСерии Тогда
		МассивОтбор.Добавить("Серии");
	КонецЕсли;
	Если Объект.ИскатьВСтатьи Тогда
		МассивОтбор.Добавить("Статьи");
	КонецЕсли;
	Если Объект.ИскатьВМноготомники Тогда
		МассивОтбор.Добавить("Многотомники");
	КонецЕсли;
	Возврат МассивОтбор;	
КонецФункции // ПолучитьМассивОтбор
...
Процедура Искать(Направление)
	...
	Попытка
		...
		Результат = ИскатьВыполнитьСервер(Направление, ТекущаяПозиция, СтрокаПоиска,ПолучитьМассивОтбор());
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Объект.ИскатьВАвторы = Истина;
	Объект.ИскатьВМноготомники = Истина;
	Объект.ИскатьВСерии = Истина;
	Объект.ИскатьВСтатьи = Истина;
	Объект.ИскатьВУчетныеЕдиницы = Истина;
	...
	Попытка
		...
		Если Не ПустаяСтрока(Параметры.ПереданнаяСтрокаПоиска) Тогда
			...
			Результат = ИскатьВыполнитьСервер(0, ТекущаяПозиция, СтрокаПоиска,ПолучитьМассивОтбор());
...
Функция ПриИзмененииТекстаСервер()
	...
	Возврат ИскатьВыполнитьСервер(0, ТекущаяПозиция, СтрокаПоиска,ПолучитьМассивОтбор());
КонецФункции
