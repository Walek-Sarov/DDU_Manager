...
Процедура УстановитьДоступностьРеквизитовРегистрации()
	...
	Если Не ЗначениеЗаполнено(Объект.ВидДокумента) И Делопроизводство.ИспользоватьВидыВнутреннихДокументов() Тогда // не известен вид нумерации
...
&НаСервере
Функция КорреспондентЮрЛицо()
	
	Возврат ЗначениеЗаполнено(Объект.Корреспондент) 
		  И ТипЗнч(Объект.Корреспондент) = Тип("СправочникСсылка.Корреспонденты") 
		  И Объект.Корреспондент.ВидКорреспондента = Перечисления.ВидыКорреспондентов.ЮрЛицо;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.КонтактноеЛицо.Доступность = КорреспондентЮрЛицо();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзмененУчетПоДетямДляВнутреннихДокументов" Тогда
...
Процедура РедактироватьСоставСвойствВыполнить()
	...
	НаборыСвойствОбъекта = УправлениеСвойствамиПереопределяемый.ПолучитьНаборыСвойствПоВидуВнутреннегоДокумента(Объект.ВидДокумента);
...
Процедура удуУстановитьВидимостьСведенийОРебенке() 	
	...
	СписокДетей = "";
	КоличествоДетей = Объект.удуДети.Количество();
	Если КоличествоДетей = 0 Тогда
		Элементы.удуСведенияОДетях.Заголовок = "Не указано";
	Иначе	
		Индекс=0;
		Пока Индекс < 3 И Индекс < КоличествоДетей Цикл
			Если СписокДетей <> "" Тогда
				СписокДетей = СписокДетей + ", ";
			КонецЕсли;
			СписокДетей = СписокДетей + Объект.удуДети[Индекс].Ребенок.Наименование;
			Индекс = Индекс + 1;
		КонецЦикла;
		Если КоличествоДетей > 3 Тогда
			СписокДетей = СписокДетей + ", ...";
		КонецЕсли;
		Элементы.удуСведенияОДетях.Заголовок = "В отношении("+ КоличествоДетей + "): "+ СписокДетей;
	КонецЕсли;
	
	Если Не Объект.Ссылка.Пустая() Тогда
		Элементы.удуСведенияОРебенкеГруппа.Видимость = Объект.Ссылка.ВидДокумента.удуУчетПоДетям;
		Элементы.удуГруппаДети.Видимость = Объект.Ссылка.ВидДокумента.удуУчетПоДетям;
	Иначе
...
&НаКлиенте
Процедура удуПоказатьСведенияОДетях(Команда)
	Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.удуГруппаДети;
КонецПроцедуры

&НаКлиенте
Процедура удуДетиПриИзменении(Элемент)
	// Вставить содержимое обработчика.
	удуУстановитьВидимостьСведенийОРебенке();
КонецПроцедуры

&НаКлиенте
Процедура удуДетиРебенокОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если удуРебенокВыбран(ВыбранноеЗначение) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Строка(ВыбранноеЗначение) + " уже включен(а) в список!");
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция удуРебенокВыбран(ВыбранноеЗначение)
	Для Каждого Строка Из Объект.удуДети Цикл
		Если Строка.Ребенок = ВыбранноеЗначение Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;	
	Возврат Ложь;
