...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Макет = Справочники.удуСемейноеПоложениеФизЛиц.ПолучитьМакет("КлассификаторСемейногоПоложения");
...
Функция НайтиЭлемент(Наименование)
	 ...
	 Возврат Справочники.удуСемейноеПоложениеФизЛиц.НайтиПоНаименованию(Наименование);
...
Процедура КлассификаторОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ТекущаяОбласть.Низ = ТекущаяОбласть.Верх Тогда
		...
		Если НЕ Ссылка.Пустая() Тогда
			...
			Вопрос = "В справочнике ""Семейное положение физических лиц"" уже существует элемент с наименованием """ + Наименование + """! Открыть существующий?";
			Ответ  = Вопрос(Вопрос, РежимДиалогаВопрос.ДаНетОтмена, , КодВозвратаДиалога.Отмена, );
			
			Если      Ответ = КодВозвратаДиалога.Да Тогда
				
				ФормаЭлемента=ПолучитьФорму("Справочник.удуСемейноеПоложениеФизЛиц.Форма.ФормаЭлемента",Новый Структура("Ключ",Ссылка),ВладелецФормы, );
		...
		ФормаНовогоЭлемента = ПолучитьФорму("Справочник.удуСемейноеПоложениеФизЛиц.Форма.ФормаЭлемента", ,ВладелецФормы, );
