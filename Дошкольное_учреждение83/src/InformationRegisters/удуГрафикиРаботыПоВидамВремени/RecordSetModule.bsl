...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Если (Отбор.План.Значение = Ложь) И (Отбор.План.Использование)
		...
		|ИЗ
		|	РегистрСведений.удуГрафикиРаботыПоВидамВремени КАК Графики
		|ГДЕ
		...
		Если ЗапросРасчетаОсновногоЗначения.Выполнить().Выбрать().Количество() = 1 Тогда
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.удуГрафикиРаботыПоВидамВремени КАК РасчетОсновногоЗначения
	...
	ИначеЕсли ТипЗнч(Отбор.ГрафикРаботы.Значение) = Тип("СправочникСсылка.удуГрафикиРаботы") Тогда
		...
		Для каждого ЗаписьНабора Из ЭтотОбъект Цикл
			...
			Если Не ТребуетсяНормаДругогоГрафика Тогда
				Если ЗаписьНабора.ВидУчетаВремени = Перечисления.удуВидыУчетаВремени.ПоДням Или ЗаписьНабора.ВидУчетаВремени = Перечисления.удуВидыУчетаВремени.ПоЧасам Тогда
		...
		Если ТребуетсяНормаДругогоГрафика Тогда
			...
			Для каждого ЗаписьНабора Из ЭтотОбъект Цикл
				...
				КонецЕсли;
				Если ЗаписьНабора.ВидУчетаВремени = Перечисления.удуВидыУчетаВремени.ПоДням Тогда 
					ЗаписьНабора.ОсновноеЗначениеНорма = НормаДней;
					ЗаписьНабора.ДополнительноеЗначениеНорма = НормаЧасов;
				ИначеЕсли ЗаписьНабора.ВидУчетаВремени = Перечисления.удуВидыУчетаВремени.ПоЧасам Тогда	
...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Если (Отбор.План.Значение = Истина) И (Отбор.План.Использование)
		...
		НаборЗаписейФакта = РегистрыСведений.удуГрафикиРаботыПоВидамВремени.СоздатьНаборЗаписей();
