...
//  Ссылка на файл.
//
Функция ПоместитьВыбранныеФайлыВХранилищеВеб(Знач ВладелецФайла, Знач НастройкиРаботыСФайлами, Знач ИдентификаторФормы) Экспорт
...
Функция ПоместитьФайлНаДискеВХранилищеВеб(Знач ДанныеФайла, ИнформацияОФайле, Знач ИдентификаторФормы)
	...
	СтрокиПути = ФайловыеФункцииСлужебныйКлиентСервер.РазложитьСтрокуПоТочкамИСлэшам(ВыбранноеИмяФайла);
	...
	Если ПодключитьРасширениеРаботыСФайлами() Тогда
		...
		Если МассивПодписей.Количество() > 0 Тогда
			...
			ПрисоединенныеФайлыСлужебныйВызовСервера.ЗанестиИнформациюОПодписях(
				ПрисоединенныйФайл, МассивПодписей, ИдентификаторФормы);
...
#Иначе
	...
	Если НЕ ИзвлекатьТекстыФайловНаСервере Тогда
		...
		АдресВременногоХранилищаТекста = ФайловыеФункцииСлужебныйКлиентСервер.ИзвлечьТекстВоВременноеХранилище(ПолныйПутьКФайлу, ИдентификаторФормы);
		УдалитьФайлы(ПолныйПутьКФайлу);
	Иначе
		АдресВременногоХранилищаТекста = "";
	КонецЕсли;
...
Функция СообщениеОПревышенииМаксимальноДопустимогоРазмераФайла(Знач РазмерФайла,
                                                               Знач МаксимальныйРазмерФайла,
