...
// Записывает присоединеннный файл.
// 
// Параметры:
...
Процедура Расшифровать(Знач ПрисоединенныйФайл, Знач РасшифрованныеДанные) Экспорт
	...
	ИначеЕсли НЕ ФайловыеФункцииСлужебный.ФоновоеИзвлечениеТекстовФайловНаСервере() Тогда
...
Процедура СкопироватьПрисоединенныеФайлы(Знач Источник, Знач Получатель) Экспорт
	...
	Для Каждого КопируемыйФайл Из КопируемыеФайлы Цикл
		...
		КопияФайла.ТипХраненияФайла = ФайловыеФункцииСлужебный.ПолучитьТипХраненияФайлов();
		Если ФайловыеФункцииСлужебный.ПолучитьТипХраненияФайлов() = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
			ЗаписатьФайлВИнформационнуюБазу(КопияФайлаСсылка, ДвоичныеДанные);
		Иначе
			// добавить в один из томов (где есть свободное место)
			ФайловыеФункцииСлужебный.ДобавитьНаДиск(ДвоичныеДанные, КопияФайла.ПутьКФайлу, КопияФайла.Том, 
...
Функция ИзвлечьТекст(Знач ДвоичныеДанные, Знач Расширение, ИзвлеченныйТекст) Экспорт
	...
	Если ФайловыеФункцииСлужебный.ЭтоПлатформаWindows()
	   И ФайловыеФункцииСлужебный.ФоновоеИзвлечениеТекстовФайловНаСервере() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Извлечение текста'"), УровеньЖурналаРегистрации.Ошибка, , , 
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
