...
Функция ВыгрузитьТекущуюОбластьВАрхив(Знач БезДеленияПоФайлам = Ложь, Знач СохранятьПаролиПользователей = Ложь) Экспорт
	...
	ПередЗавершениемВыгрузки(КаталогВыгрузки);
...
Процедура ЗаписатьДанныеОбластиВXML(Знач Запись, Знач ВыгружатьОбщиеПоставляемыеДанные = Ложь) Экспорт
	...
	Для каждого МетаданныеКонстанты Из Метаданные.Константы Цикл
		...
		Попытка
			СтандартныеПодсистемыПереопределяемый.ПередВыгрузкойДанных(МенеджерЗначения);
	...
	Для каждого МДРегистра Из Метаданные.РегистрыСведений Цикл
		...
		Попытка
			СтандартныеПодсистемыПереопределяемый.ПередВыгрузкойДанных(НаборЗаписей);
	...
	Для каждого ВидОбъекта Из ВидыОбъектов Цикл
		...
		Для каждого МДОбъекта Из МетаданныеКоллекция Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				Попытка
					СтандартныеПодсистемыПереопределяемый.ПередВыгрузкойДанных(Объект);
...
Процедура ЗагрузитьДанныеСПодменойСсылок(Знач КаталогВыгрузки, Знач РежимОтладки)
	...
	ДополнитьСловарьЗаменПоКаталогуВыгрузки(ФрагментСловаряЗамен, КаталогВыгрузки);
...
//
Процедура ПередЗавершениемВыгрузки(Знач КаталогВыгрузки)
...
//
Процедура ДополнитьСловарьЗаменПоКаталогуВыгрузки(Знач СловарьЗамен, Знач КаталогВыгрузки)
