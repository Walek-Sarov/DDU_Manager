...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	ИначеЕсли Дескриптор.DataType = "КурсыВалют" Тогда
		Если ПоставляемыеДанные.СсылкиПоставляемыхДанныхИзКэша("КурсыОднойВалюты").Количество() = 0 Тогда
			Загружать = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Знач Обработчики) Экспорт
	...
	Обработчик.Процедура = "КурсыВалютВМоделиСервиса.ПреобразованиеСвязейВалют";
...
////////////////////////////////////////////////////////////////////////////////
// Сериализация/десеривализации файла курсов
...
Процедура ОбработатьПоставляемыеКурсыЗаДень(Знач Дескриптор, Знач ПутьКФайлу)
	...
	ОбластиДляОбновления = ПоставляемыеДанные.ОбластиТребующиеОбработки(Дескриптор.FileGUID, "КурсыВалютЗаДень");
...
Процедура ЗапланироватьКопированиеКурсовВалюты(Знач Валюта) Экспорт
	...
	ПараметрыЗадания.Вставить("ИмяМетода"    , "КурсыВалютВМоделиСервиса.КопироватьКурсыВалюты");
